...
A derivatives exchange operator is porting its production trading platform from Boost.Asio callbacks to a coroutine-native I/O library built on C++20 coroutines. This paper reports the early findings from that integration: what worked, what did not, and what the experience reveals about coroutine-native I/O design in practice. The results are preliminary — the integration covers a subset of the platform and no performance benchmarks have been completed yet — but the qualitative findings from two structured interviews with the engineering team are reported here as field evidence for the committee's consideration.
...
The author is affiliated with the C++ Alliance, which develops [Capy](https://github.com/cppalliance/capy)<sup>[1]</sup> and [Corosio](https://github.com/cppalliance/corosio)<sup>[2]</sup>, the libraries under test. The integration partner is an independent commercial entity. The author has papers before the committee proposing coroutine-based I/O ([P4003R0](https://wg21.link/p4003r0)<sup>[3]</sup>) and analysing `std::execution` ([P4007R1](https://isocpp.org/files/papers/P4007R1.pdf)<sup>[4]</sup>, [P4014R0](https://wg21.link/p4014r0)<sup>[5]</sup>).

This paper documents findings. It does not propose changes, request committee time, or advocate for a position. The evidence is early-stage and the authors acknowledge its limitations explicitly in §8.
...
The interviews were qualitative. No metrics, benchmarks, or automated measurements are reported. The findings represent the subjective assessments of two experienced engineers at the early stage of an ongoing integration. The committee should weigh them accordingly.
...
The most notable finding: migrating production callback-based code to coroutines was feasible and less disruptive than anticipated.
...
Recursive callback patterns (retry loops, reconnection handlers) converted naturally to structured coroutine loops — simpler and more readable than the callback originals.
...
The approach uses an `executor_traits` abstraction layer allowing parallel Asio and Capy implementations. Tests run identically against both backends, validating behavioural equivalence. This allowed the team to port incrementally without disrupting the existing Asio codebase.
...
The exception-vs-error-code boundary is a recurring design question in C++ I/O libraries. It is particularly acute for financial markets infrastructure where exception-free code paths are a standard requirement.
...
These assessments are based on approximately two weeks of integration work covering a subset of the platform. No performance benchmarks have been completed. The endorsement should be read as an early indicator, not a final verdict.
...
This paper reports early-stage, qualitative findings. The committee should be aware of the following limitations:
